...
## output: list with the results and parameters of the lim rule analysis
	...
	if (resultlist_part$method == 'dist') {
		savelist[[savelistcounter <- savelistcounter+1]] <- list(element=element, tosave = qat_save_distribution_1d(resultlist_part, baseunit=""))
	}
	return(savelist)
}
