qat_call_save_slide_distribution <-
...
## output: list with the results and parameters of the lim rule analysis
	...
	if (resultlist_part$method == 'dist_slide') {
		if (dim_mv==1) {
			savelist[[savelistcounter <- savelistcounter+1]] <- list(element=element, tosave = qat_save_slide_distribution_1d(resultlist_part, baseunit=""))
		}
		if (dim_mv==2) {
			savelist[[savelistcounter <- savelistcounter+1]] <- list(element=element, tosave = qat_save_slide_distribution_2d(resultlist_part, baseunit=""))
		}		
	}
	return(savelist)
}
