...
## functionality: calling function for qat_analyse_set_addup
...
## output: corrected measurement_vector
	if(!is.null(workflowlist_part$blocksize)) {
		...
		if (is.null(dim(measurement_vector))) {
			resultlist[[resultlistcounter <- resultlistcounter+1]] <- list(element=element, method='addup', result =qat_analyse_set_addup_1d(measurement_vector, blocksize_value))
		}
		if (length(dim(measurement_vector))==2) {
			resultlist[[resultlistcounter <- resultlistcounter+1]] <- list(element=element, method='addup', result =qat_analyse_set_addup_2d(measurement_vector, blocksize_value))
		}
	}
	return(resultlist)
}
